...
func TestFnBuildImageSuccessfully(t *testing.T) {
	...
	// Instantiate a client
	client := NewTestClient(server.URL(), t)
	name, _, err := FnImageBuild(client, &BuildOptions{"./testing_data", "", nil, "test", "", nil})
	if err != nil {
...
func TestFnBuildImageRemoteSuccessfully(t *testing.T) {
	...
	// Instantiate a client
	client := NewTestClient(server.URL(), t)
	name, _, err := FnImageBuild(client, &BuildOptions{"./testing_data", "", nil, "test", "https://github.com/gofn/dockerfile-python-exampl://github.com/gofn/dockerfile-python-example.git", nil})
	if err != nil {
		t.Errorf("FnImageBuild expected nil but found %q, %q", name, err)
	}
...
func TestFnBuildImageDockerfileNotFound(t *testing.T) {
	...
	// Instantiate a client
	client := NewTestClient(server.URL(), t)
	_, _, err := FnImageBuild(client, &BuildOptions{"./wrong", "Dockerfile", nil, "test", "", nil})
	if err == nil {

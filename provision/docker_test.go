...
func TestFnBuildImageSuccessfully(t *testing.T) {
	...
	// Instantiate a client
	client := NewTestClient(server.URL(), t)
	name, _, err := FnImageBuild(client, &BuildOptions{"./testing_data", "", false, "test", "", "", nil, docker.AuthConfiguration{}})
	if err != nil {
...
func TestFnBuildImageRemoteSuccessfully(t *testing.T) {
	...
	// Instantiate a client
	client := NewTestClient(server.URL(), t)
	name, _, err := FnImageBuild(client, &BuildOptions{"./testing_data", "", false, "test", "https://github.com/gofn/dockerfile-python-exampl://github.com/gofn/dockerfile-python-example.git", "", nil, docker.AuthConfiguration{}})
	if err != nil {
		t.Errorf("FnImageBuild expected nil but found %q, %q", name, err)
	}
...
func TestFnBuildImageDoNotUsePrefixImageName(t *testing.T) {
	...
	name, _, err := FnImageBuild(client, &BuildOptions{"./testing_data", "", true, imageName, "", "", nil, docker.AuthConfiguration{}})
	if err != nil {
		t.Errorf("FnImageBuild expected nil but found %q, %q", name, err)
	}
	if name != imageName {
...
func TestFnBuildImageDockerfileNotFound(t *testing.T) {
	...
	// Instantiate a client
	client := NewTestClient(server.URL(), t)
	_, _, err := FnImageBuild(client, &BuildOptions{"./wrong", "Dockerfile", false, "test", "", "", nil, docker.AuthConfiguration{}})
	if err == nil {
